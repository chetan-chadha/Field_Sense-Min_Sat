...
//      mySensorData = SD.open("test.txt", FILE_WRITE);
      ...
      buf +=String(",");buf +=String(GPS.speed);
      buf1 +=String(",");buf1 +=String(GPS.angle);
      buf1 +=String(",");buf1 +=String(GPS.altitude);
      buf1 +=String(",");buf1 +=String((int)GPS.satellites);
